...
Load the `linkedin-authority-engine:authority-context` skill. Read `authority-context.md` (§2 the 3 pillars, §4 Audience, §11 content mix + posting frequency + preferred days) and `memory/`. If §2 has no pillars yet, run `linkedin-authority-engine:content-pillars` first. Operate in the profile's `language`.

## When to trigger

The user says "plan my next 4 weeks", "build my content calendar", "give me a month of posts", "I want a posting schedule", or after running `linkedin-authority-engine:content-pillars`.
...
When the user picks a slot to write, hand off to `/linkedin-authority-engine:guided` with that topic — it runs the full generation + finalization pipeline and saves the post to `outputs/posts/`.
