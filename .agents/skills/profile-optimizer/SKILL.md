---
name: linkedin-authority-engine:profile-optimizer
...
If there is no `authority-context.md`, run `/linkedin-authority-engine:init` first, then stop. (This grounds the rewrites in the client's real positioning instead of generic advice.)

## Gate 2 — Read the substrate

Load the `linkedin-authority-engine:authority-context` skill. Read `authority-context.md` (§2 Positioning, §4 Audience, §5 Offers, §1 credibility markers) and `memory/`. The rewrites must match this positioning and speak to this ICP. Operate in the profile's `language`.
...
Save to `outputs/strategy/<YYYYMMDD>-profile-audit.md`. This is an audit artifact, not a post — it does **not** go through the post finalization pipeline. The rewritten **copy** should still pass `linkedin-authority-engine:humanizer-linkedin` before the user pastes it live.
...
If the audit reveals the positioning itself is fuzzy (not just the copy), hand off to `linkedin-authority-engine:niche-definer` to sharpen it before rewriting the profile around it.
