---
name: linkedin-authority-engine:audience-persona
...
Load the `linkedin-authority-engine:authority-context` skill. Read `authority-context.md` (especially §4 Audience and §6 Commercial narrative — objections are persona gold) and `memory/`. Operate in the profile's `language`. Start from the §4 the profile already has; you are sharpening it into one named person, not starting blank.

## When to trigger

The user says "who am I writing for", "build my audience persona", "my posts talk to everyone", "define my ICP", or after running the `linkedin-authority-engine:niche-definer` skill.
...
With the persona locked, build the recurring themes that serve it with `linkedin-authority-engine:content-pillars`, then turn pillars into a schedule with `linkedin-authority-engine:content-calendar`.
