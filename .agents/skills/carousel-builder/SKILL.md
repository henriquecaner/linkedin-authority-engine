---
name: linkedin-authority-engine:carousel-builder
...
2. Optionally score the **caption** with `/Users/henriquecaner/Documents/GitHub/linkedin-content-caner/.agents/scripts/score_post.py <caption.txt> --lang <pt|en>` for **hook and CTA signal only** — do **not** apply the ≥9/10 publish gate here. A caption is a short tease, so it will score low on Structure/Data by design; that is expected, not a failure. The slides are the value payload, not the caption.
