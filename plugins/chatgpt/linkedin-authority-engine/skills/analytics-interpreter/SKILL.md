...
- Vanity metric warnings:
  ...
  - Profile visits without DMs = the bio is not converting (hand off to `profile-optimizer`).
...
If the diagnosis points to a profile that does not convert visits, hand off to `profile-optimizer`. If a format is clearly winning, plan more of it with `content-calendar`.
