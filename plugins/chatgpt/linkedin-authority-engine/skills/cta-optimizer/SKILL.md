...
The user says "fix my CTA", "my ending is weak", "how should I close this post", "the ending isn't earning comments/saves", or pastes a post that ends on a flat "Thoughts?". (For a whole-post problem, not just the close, use the `rewrite` skill.)
...
(Full bank + formulas in the `ctas` skill. 2026 priority: Saves › Long comments › Follow › Leads.)
...
If the post needs more than a CTA fix, hand off to the `rewrite` skill for a full two-version optimization, or the `score` skill to check it against all 6 dimensions.
