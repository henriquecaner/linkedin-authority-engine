...
Run `${PLUGIN_ROOT}/scripts/validate_specs.py` on the original post and analyze it against the 360Brew specs (load `360brew-algorithm` for the full reference).
...
- **Version B (Bold):** Stronger hook (use the `hooks` skill), restructures for maximum impact (use `copywriting-structures`), maximizes Saves Potential.

For the CTAs of both versions, use the `ctas` skill, aligning to the post's objective.
...
Run `${PLUGIN_ROOT}/scripts/validate_specs.py` on each version. Fix errors before advancing.
...
Run the `humanizer-linkedin` skill on each version (A and B). Present a compact diff per version (max 5 items).

### Stage C — Final score

Run `${PLUGIN_ROOT}/scripts/score_post.py` on each humanized version:
...
Run the `visual-brief` skill on the chosen version.
...
After approval of the final version, following the instructions of the `authority-context` skill:
